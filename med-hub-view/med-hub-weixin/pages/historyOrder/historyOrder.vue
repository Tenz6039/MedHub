...
<template>
  <view class="history_order">
    <uni-nav-bar @clickLeft="goBack" left-icon="back" leftIcon="arrowleft" title="历史订单" statusBar="true" fixed="true"
      color="#1D1D1F" backgroundColor="#FFFFFF" class="ios-nav-bar"></uni-nav-bar>
...
export default {
  ...
  methods: {
    ...
    // 获取历史订单列表
    getList() {
      if (!this.urlMap[this.tabIndex]) {
        console.error('urlMap 中缺少 tabIndex', this.tabIndex, '的配置');
        return;
      }
      const key = this.urlMap[this.tabIndex].key;
      const fn = this.urlMap[this.tabIndex].fn;
      if (!fn) {
        console.error('fn 未定义，tabIndex:', this.tabIndex);
        return;
      }
      const params = {
        pageSize: 10,
