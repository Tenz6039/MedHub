...
<template>
  <view class="more_norm_pop">
    <view class="title">{{ moreNormMedicinedata.name }}</view>
    <scroll-view class="items_cont" scroll-y="true" scroll-top="0rpx">
      <view class="item_row" v-for="(obj, index) in moreNormdata" :key="index">
        <view class="specification_name">{{ obj.name }}</view>
        <view class="specification_item">
          <view
            :class="{
              item: true,
              act: specificationDataes.findIndex((it) => item === it) !== -1,
            }"
    ...
    <view class="but_item">
      <view class="price">
        <text class="ico">￥</text>
        {{ moreNormMedicinedata.price }}
      </view>
      <view class="active"
        ><view class="dish_card_add" @click="addShop(moreNormMedicinedata, '普通')"
          >加入购物车</view
        ></view
      >
    </view>
    <view class="close" @click="closeMoreNorm(moreNormMedicinedata)"
      ><image
        class="close_img"
