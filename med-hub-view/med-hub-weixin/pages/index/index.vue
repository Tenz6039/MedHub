    ...
    <view class="home_content" :style="{ paddingTop: ht + 'px' }" @touchmove.stop.prevent="disabledScroll">
      ...
      <view class="restaurant_info_box">
        <view class="restaurant_info">
          ...
          <view class="info_bottom">
            <view>
              <view class="word">智慧药房为顾客打造专业的药品配送服务</view>
              <view class="address">
                <icon></icon>
                {{ shopInfo.shopAddress || "药房地址获取中.." }}
              </view>
            </view>
            <view>
      ...
      <view class="restaurant_menu_list" v-if="shopStatus === 1">
        ...
        <scroll-view class="vegetable_order_list" scroll-y="true" scroll-top="0rpx"
          v-if="medicineListItems && medicineListItems.length > 0">
          <view class="type_item" v-for="(item, index) in medicineListItems" :key="index">
            <!-- 点击查看详情 -->
            ...
            <view class="dish_info">
              ...
              <view class="dish_label" @click="openDetailHandle(item)">月销量 0</view>
              ...
              <view class="dish_active" v-if="!item.flavors || item.flavors.length === 0">
                <!-- 减药 -->
                <image v-if="item.medicineNumber >= 1" src="../../static/btn_red.png" @click="redMedicineFromCartAction(item, '普通')"
                  class="dish_red"></image>
                <text v-if="item.medicineNumber > 0" class="dish_number">{{
                  item.medicineNumber
                }}</text>
                <!-- 加药 -->
                <image src="../../static/btn_add.png" class="dish_add" @click="addMedicineToCartAction(item, '普通')"></image>
              </view>
        ...
        <view class="no_dish" v-else>
          <view v-if="typeListData.length > 0">该分类下暂无药品</view>
        </view>
      </view>
      <view class="restaurant_close">药房已打烊</view>
      <!-- end -->
      ...
      <view class="footer_order_buttom order_form" v-else>
        <view class="orderCar" @click="() => (openOrderCartList = !openOrderCartList)">
          ...
          <view class="order_price">
            <text class="ico">￥</text>
            {{ orderMedicinePrice.toFixed(2) }}
          </view>
        </view>
        <view class="order_but" :class="{ disabled: orderMedicineNumber === 0 }" @click="goOrder()">去结算</view>
      </view>
      <!-- end -->
      <!-- 选择多规格弹层 - start -->
      <view class="pop_mask" v-show="openMoreNormPop">
        <popMask :moreNormMedicinedata="moreNormMedicinedata" :moreNormdata="moreNormdata" :specificationDataes="specificationDataes"
          @checkMoreNormPop="checkMoreNormPop" @addShop="addShop" @closeMoreNorm="closeMoreNorm"></popMask>
      </view>
      <!-- 选择多规格 - end -->
      <!-- 药品详情弹层 - start -->
      <!-- openDetailHandle 这个函数触发的药品详情 -->
      <view class="pop_mask" v-show="openDetailPop" style="z-index: 9999">
        <medicine-detail :medicineDetailes="medicineDetailes" :openDetailPop="openDetailPop" :medicineCombinationData="medicineCombinationData"
          @redMedicineFromCartAction="redMedicineFromCartAction" @addMedicineToCartAction="addMedicineToCartAction" @moreNormDataesHandle="moreNormDataesHandle"
          @dishClose="medicineClose"></medicine-detail>
      </view>
      <!-- 药品详情 - end -->
      <!-- 购物车弹框 - start -->
      <view class="pop_mask" v-show="openOrderCartList" @click="openOrderCartList = !openOrderCartList">
        <popCart :openOrderCartLis="openOrderCartList" :orderAndUserInfo="orderAndUserInfo"
          @clearCardOrder="clearCardOrder" @addMedicineToCartAction="addMedicineToCartAction" @redMedicineFromCartAction="redMedicineFromCartAction"></popCart>
      </view>
      <!-- 购物车弹框 - end -->
