<template>
  <div class="selectInput">
    ...
    />
    <div v-if="mak && medicineFlavorsData.length" class="flavorSelect">
      <span
        v-for="(it, ind) in medicineFlavorsData"
        :key="ind"
        class="items"
        @click="checkOption(it, ind)"
      >{{ it.name }}</span>
      <span v-if="medicineFlavorsData == []" class="none">无数据</span>
    </div>
  </div>
</template>
...
export default class extends Vue {
  ...
  @Prop({ default: [] }) private medicineFlavorsData!: []
